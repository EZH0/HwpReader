...
보안 승인 팝업을 줄이려면 포함된 `FilePathCheckerModuleExample.dll`을 레지스트리에 등록합니다.

```powershell
reg add "HKCU\Software\HNC\HwpAutomation\Modules" /v FilePathCheckerModuleExample /t REG_SZ /d "%CD%\FilePathCheckerModuleExample.dll" /f
reg add "HKCU\Software\WOW6432Node\HNC\HwpAutomation\Modules" /v FilePathCheckerModuleExample /t REG_SZ /d "%CD%\FilePathCheckerModuleExample.dll" /f
Unblock-File ".\FilePathCheckerModuleExample.dll"
```
...
HWP 표 추출이 실패하거나 표가 아닌 값이 추출되면 엑셀 파일을 갱신하지 않고 종료합니다.
...
- `full_output_path`: 전체 결과를 갱신할 엑셀 파일 경로입니다.
- `sorted_output_path`: 주소 필터 결과를 갱신할 엑셀 파일 경로입니다.
